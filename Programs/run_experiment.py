...
parameters = ['window_size', 'window_weight', 'cooc_direction', 'normalization', 'reduction', 'sim_type']

parameter_dict = {'window_size': [1,2,7],
                   'window_weight': ['linear','flat'],
                   'cooc_direction': ['forward', 'backward', 'summed', 'concatenated'],
...
def run_experiment(num_run):
    ...
    for i in range(num_run):
        ...
        if s_task:
            ...
            standard_ranking = syntagmatic_task.get_standard_ranking(p_nouns, t_verbs, pairs)
        ...
        for model_parameters in model_para_list:
            ...
            for model in models:
                ...
                if p_task:
                    within_between = paradigmatic_task.run_task(kit,model)
                    for j in range(4):
                ...
                if s_task:
                    ...
                    current_ranking_matrix = np.concatenate((ranking_data_matrix, output_ranking), 1)
